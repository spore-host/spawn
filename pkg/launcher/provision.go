...
type Options struct {
	...
	// ContainerScript, if non-empty, installs Docker (if absent), authenticates
	// to a private ECR registry if needed, pulls an image, and runs it —
	// headlessly, no DCV/X11 (spore-host#353). Generated by
	// pkg/userdata.GenerateContainerUserData. Runs after StorageScript (so a
	// containerized workload sees any mounted volumes already live) and before
	// CustomUserData.
...
func Provision(ctx context.Context, client *aws.Client, config aws.LaunchConfig, opts Options) (*aws.LaunchResult, error) {
	...
	if config.UserData == "" {
		...
		bootstrap, err := BuildLinuxBootstrap(BootstrapConfig{
			...
			ContainerScript: opts.ContainerScript,
