...
type BootstrapConfig struct {
	...
	// ContainerScript, if non-empty, installs Docker, authenticates to a
	// private ECR registry if needed, pulls an image, and runs it — headlessly,
	// no DCV/X11 (spore-host#353). Runs AFTER StorageScript (so a containerized
	// workload sees any mounted volumes already live) and BEFORE CustomUserData.
	// Generated by pkg/userdata.GenerateContainerUserData.
...
`, string(declJSON), len(cfg.Plugins))
	...
	// sees any mounted volumes already live; before the user's own script.
	// set +e for the same reason as StorageScript — a docker pull/login hiccup
	// must not abort the whole user-data before CustomUserData even runs.
	if cfg.ContainerScript != "" {
		script += "\nset +e\n"
		script += cfg.ContainerScript
	}
